...
void TSPRec(double curr_bound, double curr_weight, int level, vector<int>& curr_path, vector<bool>& visited) {
    ...
    for (int i = 0; i < N; i++) {
        vector<int> local_path(curr_path);
        vector<bool> local_visited(visited);

        if (adj[local_path[level - 1]][i] != INT_MAX && !local_visited[i]) {
            double temp_curr_weight = curr_weight + adj[local_path[level - 1]][i];
            double temp_curr_bound = curr_bound;

            if (level == 1)
                temp_curr_bound -= ((firstMin(local_path[level - 1]) + firstMin(i)) / 2);
            else
                temp_curr_bound -= ((secondMin(local_path[level - 1]) + firstMin(i)) / 2);

            if (temp_curr_bound + temp_curr_weight < final_res) {
                local_path[level] = i;
                local_visited[i] = true;

                // Llamada recursiva en un hilo separado
                TSPRec(temp_curr_bound, temp_curr_weight, level + 1, local_path, local_visited);
            }
        }
    }
}
...
void TSP() {
    ...
    // Paralelizar el cálculo del límite inferior inicial para el camino
